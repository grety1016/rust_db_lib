...
#[tokio::test]
#[ignore]
async fn test_mysql_full_api() -> Result<(), mysql::Error> {
    ...
    conn.exec(
        "CREATE TABLE IF NOT EXISTS full_api_test (id INT PRIMARY KEY, name VARCHAR(50), age INT)",
    )
    .await?;
    ...
    let rows = conn
        .exec(mysql::sql_bind!(
            "INSERT INTO full_api_test (id, name, age) VALUES (?, ?, ?)",
            1,
            "Alice",
            30
        ))
    ...
    assert_eq!(
        conn.query_scalar_i16("SELECT CAST(10 AS SIGNED)")
            .await?
            .unwrap(),
        10
    );
    assert_eq!(conn.query_scalar_u8("SELECT 255").await?.unwrap(), 255);
        ...
        let res = conn
            .scoped_trans::<_, ()>(async {
                conn.exec("INSERT INTO full_api_test (id, name, age) VALUES (5, 'Eve', 28)")
                    .await?;
                Err(mysql::Error::custom("Simulated failure"))
            })
            .await;
        assert!(res.is_err());
    ...
    let res: Result<(), mysql::Error> = conn
        .sandbox_trans(async {
