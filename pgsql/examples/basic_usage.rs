...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    conn.exec(sql_bind!(
        "INSERT INTO users_example (name, email) VALUES ($1, 'init@example.com')",
        user_name
    ))
    .await?;
    ...
    conn.scoped_trans(async {
        // 插入数据
        conn.exec(sql_bind!(
            "INSERT INTO users_example (name, email) VALUES ($1, 'john@example.com')",
            "John"
        ))
        .await?;
        ...
        conn.scoped_trans(async {
            conn.exec(sql_bind!(
                "UPDATE users_example SET email = $1 WHERE name = $2",
    ...
    conn.sandbox_trans(async {
        conn.exec(sql_bind!(
            "INSERT INTO users_example (name, email) VALUES ($1, 'sandbox@example.com')",
            "SandboxUser"
        ))
        .await?;
        Ok::<(), pgsql::Error>(())
    ...
    let count: i64 = conn
        .query_scalar_i64(sql_bind!(
            "SELECT count(*) FROM users_example WHERE name = $1",
            "SandboxUser"
        ))
        .await?
        .unwrap_or(0);
