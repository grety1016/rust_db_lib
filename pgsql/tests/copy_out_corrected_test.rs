...
#[tokio::test]
async fn test_copy_out_performance_1m_rows_corrected() {
    init();
    ...
    // 创建一个包含几个字段的表用于测试
    let create_sql = "CREATE TABLE IF NOT EXISTS large_table_test_corrected (
    ...
    conn.exec("TRUNCATE TABLE large_table_test_corrected RESTART IDENTITY")
        .await
        .unwrap();
    ...
    let sql = "COPY large_table_test_corrected (col1_text, col2_text, col3_text, col4_text, col5_text) FROM STDIN WITH CSV";
    ...
    let actual_count: i64 = conn
        .query_scalar_i64("SELECT COUNT(*) FROM large_table_test_corrected")
        .await
        .unwrap()
        .unwrap();
    ...
    let sql = "COPY large_table_test_corrected TO STDOUT WITH CSV";
    ...
    // 验证数据完整性
    conn.exec("DROP TABLE IF EXISTS large_table_test_corrected")
        .await
        .unwrap();
